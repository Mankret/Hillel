...
def analyze():
    if start.isdigit():
        return 'Вы ввели положительное целое число: ', int(start)
    elif re.fullmatch(r'\d*[ ,.]\d+', start):
        return 'Вы ввели положительное дробное число: ', float(start.replace(',', '.'))
    elif re.fullmatch(r'-\d*[,.]\d', start):
        return 'Вы ввели отрицательное дробное число: ', float(start.replace(',', '.'))
    elif re.fullmatch(r'-\d+', start):
        return 'Вы ввели отрицательное целое число: ', int(start)
    elif not start.isdigit():
        return 'Вы ввели не корректное число: ', start


while True:
    start = input("выход, exit, quit, e, q или введите число : ")
    if start.lower() in ('выход', 'exit', 'quit', 'e', 'q'):
        break
    print(*analyze())
